...
@section('content')
    ...
    <div class="page-content container">
        <div class="wrapper">
            <div class="row align-items-center">
                ...
                <div class="col-md-3">
                    @can('create-projects')
                        <a href="{{ route('projects.create') }}" class="btn btn-primary float-end"><i class="bi bi-plus-circle"></i>
                            Add New Project</a>
                    @endcan
                </div>
            </div>
            <div class="whitebox">
                <table id="projectList" class="table table-striped nowrap" style="width:100%">
                    <thead>
                        <tr>
    ...
    {{--@push('javascript')
        <script>
            const positions = @json($projectRelations);
            ...
            const organisationData = positions.map(position => ({
                ...
                status: position.status == 1 ?
                    '<input type="button" class="project_status btn btn-success" value="Active">' :
                    '<input type="button" class="project_status btn btn-secondary" value="Disabled">',
            ...
            (position.is_equipment == 1 ?
               ...
               `<form id="deleteForm${position.id}" action="{{ route('projects.destroy', '') }}/${position.id}" method="post">
                @csrf
                @method('DELETE')
                <a href="{{ url('projects') }}/${position.id}/edit" class="link-primary"><i class="fa-regular fa-pen-to-square"></i></a>
                <button type="submit" class="link-danger" onclick="return confirm('Do you want to delete this project?');"><i class="fa-solid fa-trash-can"></i></button>
            </form>`)  
            }));
            
            // Initializing TreeTable with the extracted data
            $('#projectList').treeTable({
